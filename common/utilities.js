...
var Utilities = {
  ...
  /*basicData: function () {
    ...
    return sessionStorage.school && sessionStorage.user ? Promise.all([]) : Promise.all([
      new Promise(function (resolve,reject) {
        ajax($api.school.get, { _fields: 'id,name,groupid,typeid,shortname',_relatedfields: 'transferstate.*'}, function (result) {
      ...
      new Promise(function (resolve,reject) {
        ajax($api.user.get, {
          _relatedfields: 'school.*,school.student.*,roles'
        }, function (result) {
          window.IGrow.user = result['data'];
          sessionStorage.user = JSON.stringify(window.IGrow.user);
          IGrow.user.uid && ajax($api.role.check, {
            uid: IGrow.user.uid,
            rolecodes: 'school.admin,school.master,school.vice_master,school.sysadmin,school.moral.master,schoolgroup.admin'
          }, function (result) {
            window.IGrow.user.school.manager = {};
            window.IGrow.user.role = {};
            result.data && result.data.roles && result.data.roles.length && result.data.roles.forEach(function (item) {
              window.IGrow.user.school.manager[item.replace('school.', '')] = true
              window.IGrow.user.role[item] = true
            });
            var getAllNews = false;
            ['school.sysadmin', 'school.admin', 'school.master', 'school.vice_master', 'school.moral.master', 'schoolgroup.admin'].forEach(function (code) {
              IGrow.user.role[code] && !getAllNews && (getAllNews = true);
            });
            IGrow.user.getAllNews = getAllNews;
            sessionStorage.user = JSON.stringify(window.IGrow.user);
            resolve(result);
          })
        },function (result) {
  ...
  basicData : function () {
    ...
    return sessionStorage.school && sessionStorage.user ? Promise.all([]) : Promise.all([
      new Promise(function (resolve) {
        ajax($api.school.get, {
          _fields: 'id,name,groupid,typeid,shortname',
  ...
  wxApiData:function (isapp,schoolid,platformid) {console.log(isapp,schoolid,platformid)
  ...
  getStyle:function(typeStyle,schoolid){
    var newTplid = typeStyle.tplid == 999 ? '999' :'0'+ typeStyle.tplid,style;
    if(typeStyle.tplid==1) {
      ...
      return new Promise(function(resolve,reject){
        Utilities.getJson('http://m-static.igrow.cn/' + newTplid + 'tpl/json/data.json',function (rlt) {
          ...
          Utilities.log(tempJson,'0'+ typeStyle.tplid + '-tpl.json');
          if(typeStyle.jsonpath){
            Utilities.getJson('http://m-static.igrow.cn'+ typeStyle.jsonpath,function (result) {
  ...
  getCateGory : function(typeid,schoolid,userBind,isNotice){
    return new Promise(function (resolve,reject) {
      axios.get($api.yzoneDatasource[userBind ? 'proxyGet' : 'oproxyGet'],{params:{
        source:'category',
        schoolid :schoolid,
        id: typeid,
        isauth :isNotice ? 0 : 1,
  ...
  getUnread:function (unids,schoolid,isNotice) {
    return new Promise(function(resolve){
      axios.get($api.yzoneCategory.unread,{params:{schoolid:schoolid,level:1,isauth:isNotice ? 0 : 1,categoryid:unids.join(',')}}).then(function (result) {
        var unreadData = result.data.data || [];
        resolve(unreadData);
      }).then(function (error) {
        resolve(error);
      })
  ...
  isObject:function (x) { return (typeof x === "function" || (typeof x === "object" && x !== null)); },
